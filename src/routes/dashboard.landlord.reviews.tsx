...
import { reviews as seed, landlordProperties, type Review } from "@/lib/landlord-data";
...
function ReviewsPage() {
  ...
  const reply = (id: string) => {
    ...
    if (!text) return;
    setItems((prev) => prev.map((r) => (r.id === id ? { ...r, reply: text } : r)));
    setDrafts((d) => ({ ...d, [id]: "" }));
          ...
          <article key={r.id} className="rounded-2xl border border-border bg-card p-5">
            <div className="flex items-start justify-between gap-3">
              <div className="flex items-center gap-3 min-w-0">
                <span className="grid place-items-center h-10 w-10 rounded-full bg-primary text-primary-foreground text-xs font-bold shrink-0">
                  {r.tenant.split(" ").map((n) => n[0]).slice(0, 2).join("")}
                </span>
                <div className="min-w-0">
                  <div className="font-semibold truncate">{r.tenant}</div>
                  <div className="text-xs text-muted-foreground truncate">{propMap[r.propertyId]} • {r.date}</div>
                </div>
              </div>
            ...
            {r.reply ? (
              <div className="mt-3 rounded-xl bg-primary-soft p-3">
                ...
                <p className="text-sm mt-1">{r.reply}</p>
              </div>
            ) : (
              <div className="mt-3 space-y-2">
                ...
                <div className="flex justify-end">
                  <Button size="sm" onClick={() => reply(r.id)}>Post reply</Button>
                </div>
              </div>
            )}
          </article>
